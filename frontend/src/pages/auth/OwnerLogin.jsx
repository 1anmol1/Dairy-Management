...
const ForgotSend = ({ onBack, onSent, role }) => {
  ...
  const doSend = async () => {
    ...
    try {
      ...
      setResult(data);
      toast.info(isMarathi ? 'रीसेट कोड तयार झाला.' : 'Reset code generated. Check the server console.');
  ...
  return (
    <div style={{ backgroundColor: '#FFFFFF', border: '1px solid #E0E0E0', padding: '36px 32px' }}>
      <button onClick={onBack} style={{
        ...
        border: 'none', cursor: 'pointer', color: '#525252', fontSize: '13px', marginBottom: '20px', padding: 0
      }}>
...
const ForgotVerify = ({ onBack, prefillIdentifier = '' }) => {
  ...
  return (
    <div style={{ backgroundColor: '#FFFFFF', border: '1px solid #E0E0E0', padding: '36px 32px' }}>
      <button onClick={onBack} style={{
        display: 'flex', alignItems: 'center', gap: '6px', background: 'none',
        border: 'none', cursor: 'pointer', color: '#525252', fontSize: '13px', marginBottom: '16px', padding: 0
      }}>
...
// ── Main component ────────────────────────────────────────────
const STEP = { LOGIN: 'login', FORGOT_SEND: 'forgot_send', FORGOT_VERIFY: 'forgot_verify' };
...
const OwnerLogin = () => {
  ...
  const subtitles = {
    [STEP.LOGIN]:         isMarathi ? 'तुमच्या फोन नंबरने साइन इन करा' : 'Sign in with your phone number',
    [STEP.FORGOT_SEND]:   isMarathi ? 'तुमचा पासवर्ड रीसेट करा'        : 'Reset your password',
    [STEP.FORGOT_VERIFY]: isMarathi ? 'OTP आणि नवीन पासवर्ड टाका'      : 'Enter OTP and new password',
  };
  ...
  return (
    ...
    >
      {step === STEP.LOGIN && (
        <OwnerLoginForm onForgot={() => setStep(STEP.FORGOT_SEND)} />
      )}
      {step === STEP.FORGOT_SEND && (
        <ForgotSend
          role="owner"
          onBack={() => setStep(STEP.LOGIN)}
          onSent={(id) => { setForgotId(id); setStep(STEP.FORGOT_VERIFY); }}
        />
      )}
      {step === STEP.FORGOT_VERIFY && (
        <ForgotVerify onBack={() => setStep(STEP.FORGOT_SEND)} prefillIdentifier={forgotId} />
      )}
    </LoginShell>
  );
};
